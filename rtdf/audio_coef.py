...
def formulario_audio_analysis(path, audio_name,timestamp=datetime.today()):
    sound = parselmouth.Sound(os.path.join(settings.MEDIA_ROOT,'audios_form',path))
    f0min=75
    f0max=300
    pointProcess = praat.call(sound, "To PointProcess (periodic, cc)", f0min, f0max)
...
def ingesta_audio_analysis(path, audio_name,timestamp=datetime.today()):
    sound = parselmouth.Sound(os.path.join(settings.MEDIA_ROOT,'audios_ingestas',path))
    f0min=75
    f0max=300
    pointProcess = praat.call(sound, "To PointProcess (periodic, cc)", f0min, f0max)
